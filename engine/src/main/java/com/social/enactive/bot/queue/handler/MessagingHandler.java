...
public class MessagingHandler {
	...
	public void handleMessage(Message message) {
		...
		try {
			String rawMessages = JacksonMapper.standardMapper().toJson(engine.process(conversationService.find(message.getConversationId()), message));
			rabbitTemplate.convertAndSend(messageDeliver, message.getConversationId(), rawMessages);
		} catch (Exception e) {
